...
export const ROLE_INSTRUCTIONS = {
  orchestrator:
    ...
    + '(2) Use orchestrator.status for the canonical lane tree + next required tool ("what is happening") view of the lanes Orca manages. Poll it while you work — it keeps your ownership from going stale. Every accepted call also renews your tool lease, and a connection configured with a refresh credential replaces a lapsed lease by itself; if a call still fails, its error names the one command that fixes it.\n'
    + '(3) Spawn scoped workers with executor.spawn { title, executorType, taskPrompt, model, permissionsProfile, intelligenceProfile, approved } — each runs under a hard, server-enforced contract (sandbox/permissions profile, worktree) in your project cwd. Choose the model for each lane HERE: model is passed to that CLI as --model (omit it for the CLI default), so never launch an agent outside Orca to get a different model — an outside worker has no lane, no isolation, and no audit. For a Codex scout pass permissionsProfile "read-only" (a read-only sandbox; the lane also counts as a non-writer). The default policy refuses a spawn without approved: true, and a spawn past your capacity is refused with 409, not queued. Right after this first spawn, start the watcher named in this response\'s (and orchestrator.register\'s) "watch" field, under a Monitor-style/streaming-output tool of your own: a finished lane only enqueues a wakeup event.drain can read, nothing else pulls it, and a session that is woken only by its own tool output needs that watcher\'s printed lines to actually notice — it prints one line per real lane state change and exits on its own once you have no active lane left.\n'
    ...
    + '(6) Enforce the completion contract yourself: AUDIT every finished executor, then make ONE verdict call — audit.accept (with findings or reviewedFiles), audit.request_fix, or audit.block (with a reason). audit.findings.record is that same verdict call with a required verdict field and applies it immediately, so never follow it with audit.accept: an accepted lane refuses a second accept. audit.queue_one queues a lane for audit when auto-audit has not. Require evidence (lane.artifacts.list / lane.artifacts.get for screenshots and transcripts, changed files, a verification run) for UI, browser, or artifact changes before you accept — never treat an executor summary as final.\n'
    ...
    + '(8) Land the work: lane.integrate merges an accepted isolated lane back into the base branch once its executor has exited and its changes are committed on the lane branch (a worktree with uncommitted changes is refused); lane.worktree.discard throws it away. fleet.emergency_stop is the break-glass stop for everything under you.\n'
    ...
    + 'Name your project with project.rename { name } when its folder name is not what the work is called — the dashboard shows a project as its directory basename until someone says otherwise, and you are the one who knows. It changes the DISPLAY NAME only (identity stays realpath(cwd), the folder on disk is untouched) and only on the project you are registered on; take projectId from your orchestrator.register response or orchestrator.status.\n'
  ...
  executor:
    ...
    + 'In an isolated worktree, commit your scoped changes on the lane branch before you submit — integration merges commits only. '
  ...
  auditor:
    ...
    + 'Queue work with audit.queue_one, then make ONE verdict call with specific findings: audit.accept, audit.request_fix, or audit.block (with a reason). audit.findings.record with a verdict is that same call and applies it immediately, so do not follow it with audit.accept. '
