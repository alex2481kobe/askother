...
func TestResolveCaller(t *testing.T) {
	cases := []struct {
		...
		{"ASKOTHER_CALLER and ASKOTHER_RUN_ID are ignored", ClientClaudeCode, claudeMeta,
			map[string]string{"ASKOTHER_CALLER": "me", "ASKOTHER_RUN_ID": "run-1", "CLAUDE_CODE_SESSION_ID": "cl"},
			"cl", "claude"},
		{"ASKOTHER_RUN_ID alone is not an identity", ClientCodex, `{}`,
			map[string]string{"ASKOTHER_RUN_ID": "run-1"}, "fb", "fallback"},
